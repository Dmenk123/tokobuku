...
class Mod_set_menu_adm extends CI_Model
{
	...
	var $order = array('m_menu.nama_menu' => 'asc'); // default order 
	...
	{
		$column = array(
			'm_level_user.id_level_user',
			'm_level_user.nama_level_user',
			'm_level_user.keterangan_level_user',
			null,
		);
	...
	{
		$q = $this->db->query("SELECT MAX(id_menu) as kode_max from m_menu");
            $kd = "";
            if($q->num_rows()>0){
				$kd = $q->row();
