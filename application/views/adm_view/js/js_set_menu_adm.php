...
$(document).ready(function() {
	...
	table = $('#tabelMenu').DataTable({
		...
		"ajax": {
			"url": "<?php echo site_url('admin/set_menu_adm/list_menu') ?>",
			"type": "POST" 
		},
    ...
    $(document).on('click', '.btn_edit_status', function(){
        if(confirm('Apakah anda yakin ubah status Menu ini ?'))
        {
            ...
            $.ajax({
                url : "<?php echo site_url('admin/set_menu_adm/edit_status_menu')?>/"+id,
                type: "POST",
                dataType: "JSON",
                data : {status : status},
...
function add_menu()
{
    ...
    var url = "<?php echo site_url('admin/set_menu_adm/get_parent_data')?>";
    $.ajax({
        type: "GET",
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('admin/set_menu_adm/add_data_menu')?>";
    }
