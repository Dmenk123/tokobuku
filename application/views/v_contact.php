<main class="ps-main">
<div class="ps-contact ps-section pb-80">
        <!--<div id="contact-map" data-address="New York, NY" data-title="Sky Store!" data-zoom="17"></div>-->
        <div class="ps-container">
          <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 ">
                  <div class="ps-section__header mb-50">
                    <h2 class="ps-section__title" data-mask="Contact">- PERTANYAAN UNTUK KAMI</h2>
                    <form class="ps-contact__form" action="do_action" method="post">
                      <div class="row">   
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 ">
                              <div class="form-group">
                                <label>Name <sub>*</sub></label>
                                <input class="form-control" type="text" placeholder="">
                              </div>
                            </div>
                            <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12 ">
                              <div class="form-group">
                                <label>Email <sub>*</sub></label>
                                <input class="form-control" type="email" placeholder="">
                              </div>
                            </div>
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 ">
                              <div class="form-group mb-25">
                                <label>Your Message <sub>*</sub></label>
                                <textarea class="form-control" rows="6"></textarea>
                              </div>
                              <div class="form-group">
                                <button class="ps-btn">Send Message<i class="ps-icon-next"></i></button>
                              </div>
                            </div>
                      </div>
                    </form>
                  </div>
                </div>
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 ">
                  <div class="ps-section__content">
                    <div class="row">
                          <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 ">
                            <div class="ps-contact__block" data-mh="contact-block">
                              <header><meta http-equiv="Content-Type" content="text/html; charset=euc-jp">
                                <h3>INDONESIA <span> Surabaya </span></h3>
                              </header>
                              <footer>
                                <p><i class="fa fa-map-marker"></i>Pakuwon City, Surabaya</p>
                                <p><i class="fa fa-envelope-o"></i><a href="support@store.com">support@store.com</a></p>
                                <p><i class="fa fa-phone"></i> +62 813 2434 5334</p>
                                <p><i class="fa fa-fax"></i> +62 31 434 53</p>
                                <p>&nbsp;</p>
                                <p>&nbsp;</p>
                                <p>&nbsp;</p>
                                <p>&nbsp;</p>
                              </footer>
                            </div>
                          </div>
                    </div>
                  </div>
                </div>
          </div>
        </div>
      </div>
     </main>